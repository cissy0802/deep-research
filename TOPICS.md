...
### 15. AI 带来的硬件短缺与电力短缺:真实规模与持续时间 📌
- **一句话**:GPU/HBM 供不应求和数据中心电力缺口,是短期瓶颈还是十年级基础设施危机?
- **为什么适合**:厂商产能规划、电力公司预测、政府能源政策三方口径差异极大,媒体叙事在"严重短缺"与"即将缓解"之间反复横跳;一手数据(SEC 申报、IEA/EIA 报告、公用事业委员会听证)全部公开,对抗验证增值显著。
- **理论底座**:半导体产能扩张周期(晶圆厂建设 2-4 年、先进封装瓶颈)、电力基础设施投资周期(发电 3-7 年、输电 5-10 年)、Jevons 悖论在算力领域的适用性(效率提升是否被需求增长吞噬)。
- **关键争议/正反**:NVIDIA/TSMC/SK Hynix 扩产时间表 vs 需求增长曲线(训练→推理的结构性转移是否改变需求形态);IEA 数据中心用电翻倍预测 vs 效率改进派(PUE 下降、液冷、推理芯片能效比提升);核能复兴叙事(SMR 时间表 vs 实际许可/建设进度);各地区差异(美国电网约束 vs 中东/北欧的电力套利)。
- **实证锚点**:IEA/EIA 数据中心用电预测与实测、TSMC/Samsung 晶圆厂产能时间表与实际良率、PJM/ERCOT 等电网运营商的互联排队数据、微软/Google/Amazon 核能 PPA 的实际交付时间表、HBM 产能与 GPU 出货量的供需缺口测算(Trendforce/Omdia)。
- **注意**:数据变化极快(每季财报更新产能指引),开跑前刷新到当月;区分"已签约产能"与"实际可交付产能";电力侧区分"规划中"与"已获许可/在建"。
