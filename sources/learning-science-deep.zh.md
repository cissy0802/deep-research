...
1. **一档(证据厚、课堂成立、偏倚检验干净):检索练习、间隔练习。**条件也锁死:检索要带反馈、看延迟后测,事实性/词汇类材料最稳;间隔在轻剂量塞进常规课堂时可能只剩两个百分点——它们是最可靠的,不是无条件的。
2. **二档(方向稳、幅度或边界存疑):交错练习(数学/类别学习强,词汇反向)、主动学习(读作"有引导的认知参与";0.47 SD 打折读)、worked examples(新手专用,随专长反转)。**
3. **三档(内核成立、包装是神话):刻意练习。**结构化练习必要且回报大,但勘误后的账本是:练习量解释约 14% 的表现方差,职业域不显著;"一万小时"是畅销书发明,预注册复现里顶尖组根本没练得更多。
4. **四档(合格检验反复失败):学习风格匹配教学、无引导的纯发现式学习。**前者最有利的 meta 也只有边缘小效应+26% 互作率+低质量证据;后者是两大对立学派唯一的共识敌人。
5. **注脚:重读与划线是"低效用"不是"无效"**——Dunlosky 等 2013 年的原口径是收益条件受限、不普适(它们恰是学生自报最常用的);该评级至今没有正式的十年更新,中效用三项(精细追问、自我解释、交错)的证据在 2013 年后已明显增厚。【已验证】

## 9. 十二个可检验主张

按证据强度排序:

1. **检索练习与间隔练习是全领域证据最厚的两项技巧:多个 meta 收敛、课堂成立、出版偏倚五法检验干净。**(强:Rowland/Yang/Cepeda/Latimier 均验证)
2. **对照组类型决定宣传数字:同一技巧对无活动 0.610、对重读 0.330、对精细加工策略 0.095 且不显著——读任何效应量先问跟谁比。**(强:Yang 2021 验证)
3. **测试的红利需要延迟才显现(当天重读反而赢),且自信与实效反向——"学习时的表现"不是学习。**(强:R&K 2006 与 Rowland 调节均验证)
4. **"复习间隔必须递增"无实证支持:扩张 vs 均匀 g=0.034 不显著;最优间隔只是随保持期变化的粗粒度比例(1 周延迟约 20-40%,1 年约 5-10%)。**(强:Latimier/Cepeda 2008 验证)
5. **实验室→课堂衰减真实且分技巧:检索对重读 0.50→0.33;间隔轻剂量进常规课堂只剩约 2 个百分点;交错是罕见例外(预注册课堂 RCT d=0.83)。**(中强:Bego/Rohrer 验证,应用 meta 上限未验证)
6. **交错有硬边界:材料需有可混淆性——词汇学习反向(g=-0.39),且交错作业更耗时。**(强:Brunmair & Richter 验证)
7. **刻意练习解释力有限:勘误后总体 14%(86% 在练习量之外),职业域不显著;预注册复现中顶尖组累计练习不多于优秀组(p=0.364)。**(强:2018 勘误与 2019 复现均验证)
8. **练习量本身高度可遗传(40-70%),控制遗传后练习与音乐能力不再相关——"天赋 vs 练习"的正确形态是交互模型。**(中强:Mosing 验证;2024 交互研究未验证)
9. **学习风格匹配教学在合格检验下反复失败,而 89-94% 的教育者相信它,且师训体系是主要传染源——这是流行度与证据倒挂的极端案例。**(强:Pashler/流行度双源/2024 修正 meta 均验证)
10. **主动学习方向稳、数字虚:0.47 SD 站在 88% 准实验、0 篇全控制的证据堆上;实测多学 0.46 SD 的学生自感少学 0.56 SD——学生评教会系统性惩罚有效教法。**(强:Freeman/Martella/Deslauriers 均验证)
11. **两大对立学派的实际共识:起作用的是认知参与+教学引导;新手需要例题与结构,熟手才受益于"合意困难"——同一方法随专长反转。**(中:理论综合,单项引文未验证)
12. **教育研究的元层缺陷是系统性的:复现研究占 0.13%,田野效应量中位数 0.10 SD,最著名的效应量排行榜有算术级错误——任何"效应量排行榜"先查口径再引用。**(强:Makel & Plucker/Kraft/Bergeron 均验证)
