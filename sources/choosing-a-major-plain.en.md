...
> This is the condensed version of [the deep dive of the same name](https://hub.cissychen.com/deep-research/choosing-a-major-deep.en.html). Every key number has been checked back against primary sources; for the full argument, the evidence grade on each conclusion, and the citations, read the deep dive. Figures as of July 2026.
...
- Calling whether an occupation will grow or decline — in the 2006–2016 round, it got that right 78% of the time. That is useful.
- Calling whether an occupation will grow faster than the economy as a whole — only 57%. **That is a two-way question; a coin gets 50%.**
- Across more than 700 detailed occupations, its total error was only slightly better than a model that predicts nothing at all ("assume each occupation's employment share stays unchanged"), and it beat that lazy benchmark in only 58% of occupations.
...
- BLS builds its projections on a full-employment assumption and **states explicitly that it does not predict recessions**. It also writes that its methods are "not designed to capture extremely rapid technological change."
- It has never had a real rival. BLS itself writes that no independent occupational projection comparable to it — and not derived from it — exists anywhere.
- Canada, the EU, and the UK all publish ten-year projections of this kind, and **for all three, no public quantification of ex-post error can be found**. This is not an American problem; it is a problem of the trade — BLS is in fact one of the few willing to grade itself.
...
An honest note in passing: the party that judged the old method to have "statistical and conceptual issues" is the very BLS that carried out the change — it is both the executor and the only judge. We found no independent public challenge to the new method, but "not found" is not the same as "does not exist."
...
- **Nursing's underemployment rate is the lowest in the table, but its unemployment rate ranks only 10th and is not distinctive.** So "nursing is a sure thing" can be hung on "the work matches the degree"; hang it on "you won't be unemployed" and the data will contradict you. Nursing's mid-career earnings rank only 32nd of 73 majors, and its early-to-mid-career increase is below the all-majors average.
- **Education fields take all three lowest unemployment slots** (special education, other education, and elementary education all around 1.2%), but their underemployment rates are not low.
...
- **The data are self-reported by the law schools, and employment rates directly determine rankings and admissions.** The bar association randomly re-audits only about 5% of schools a year, and audits whether supporting documents exist, not whether the categorization is right; that spot-check mechanism only started in 2015 — **when the class of 2013 data were generated, there were no spot checks at all.** The bias runs upward at both ends, so **the difference is more reliable than the level.**
- The measurement point changed in between, so one to two percentage points of the gap between 57% and 83% is an artifact of the definitional change.
- The class of 2025 is "rate up, volume down": alongside the rising ratio, the absolute number in full-time long-term employment fell from 33,931 to 31,743.
...
- **First cycle**: peak 42,337 in 1985-86 → trough 24,506 in 1995-96, **a fall of 42%** — and the bottom was a five-year plateau, not a point.
- **Second cycle**: peak 59,488 in 2003-04 → trough 37,992 in 2008-09, **a fall of 36%**.
- **Third ascent**: 39,593 in 2009-10 → 108,503 in 2021-22, **up 186%**.
...
- **US law**: an accredited school plus a bar exam, and it is the textbook case of oversupply.
- **US pharmacy**: accreditation, state licensure, and mandatory experiential rotations — all three — yet training units nearly doubled between 2000 and 2015, producing the famous pharmacist glut.
- **Nurse practitioners**: preceptors and licensure required, and program counts and output still grew several-fold within a decade.
...
**But "China has no official employment data by field of study" is too strong a statement.** Employment data by field do exist inside the system, are collected, and **are being used for administrative action** (red and yellow cards, orders to suspend admissions, program abolition). The authorities have also publicly released **ranked outputs** — they simply **never release the denominators or the values**:

- In 2014 the Ministry of Education published a *List of Undergraduate Programs with Low Employment Rates over the Past Two Years* — 15 programs nationally plus provincial lists — **giving only program names and not a single employment-rate figure**;
- In April 2026 the Ministry of Education stated that it had "pushed all provinces to publish urgently-needed-program lists and program-warning lists covering 473 program types";
- Policy documents explicitly require a "red- and yellow-card alert system for programs with low employment quality."
...
- **"Employment through other forms of hiring"**: the employer signs no employment agreement or labor contract; a certificate of engagement or wage income statements suffice, so long as pay meets the local minimum wage standard.
- **"Freelance work"** (the occupational examples include "e-sports worker"): the verification basis is "**supporting materials signed and confirmed by the graduate themselves, adjudicated by the responsible officers of the employment offices at the university and school levels.**"
- **"Self-employment / entrepreneurship"** in the e-commerce case: verified on the basis of the online shop's URL, screenshots, and income statements, **with no minimum wage threshold.**
...
- **Red card = the top ten by absolute number of unemployed graduates.** On the 2010 red-card list, English was 1st, computing 2nd, and law 3rd — **precisely the three largest programs by graduate volume that year.** This criterion is mechanically biased against large programs.
- **In the firm's own definitions, the wording for the red card and the yellow card differs by only five characters** (the yellow card is the same sentence with "other than the red-card programs" prepended). **There is no substantive criterion distinguishing red from yellow at all; there is only a ranking cut line.**
- **So structurally, the lists cannot distinguish "this program is bad" from "this program is big."**
- A clean fact that depends on no reversal narrative: the nine programs given **green cards** in 2010 and 2011, with a recommendation to "increase admissions accordingly," are all resource-extraction and heavy industry (geological engineering, naval architecture and ocean engineering, petroleum engineering, mining engineering, and others) — **today prime "avoid" territory. On this side there is no disclaimer from the publisher; the reversal is clean and one-directional.**
- **An extrapolation assumption the publisher wrote down and that was subsequently falsified**: both the 2010 and 2011 editions state verbatim that "since 2000 the enrollment structure across broad undergraduate program categories has been essentially fixed. On that premise, a program's employment and unemployment situation has a certain inertia, and absent intervention, fundamental change is unlikely." **The entire early-warning power of the lists rests on that sentence, and the fifteen years since have directly falsified it.**
...
**First, the nature of this list, stated flatly: this is how this article organizes the evidence, not any institution's conclusion.** It is not ordered by hot and cold, it assigns no score to any field, and it does not predict which field will do well in ten years. What it does is apply the conclusions above to three **transferable criteria** — **the criteria matter more than the list, because in four years the list will be stale and the criteria will not.**
...
| Field / direction | Position | Basis |
|---|---|---|
| Nursing, rehabilitation therapy, medical imaging technology, and other **hands-on medical care** | Low-codability end (robust) | Two vendor log measurements and one independent measurement using no AI product logs at all agree in placing nursing in the low-exposure group; turning a patient, transferring, drawing blood, and monitoring vital signs cannot in principle appear in a text conversation |
| **K–12 education** | Low end, but the gap from the row above is far smaller than popular claims suggest | The only non-vendor measurement places primary school teachers, secondary school teachers, nursing professionals, and general practitioners in **the same** "unexposed" category |
| **Postsecondary teaching, writing, translation, junior legal drafting, general copywriting** | High-codability end | Explaining and drafting materials are the default output form of a chatbot to begin with; three datasets consistently show postsecondary and graduate teaching markedly more exposed than K–12 teaching. **No rankable number is given in this column** |
...
| Field / direction | Position | Basis |
|---|---|---|
| **Nursing, medicine, and engineering fields requiring labs or accredited beds** | Expansion constrained physically and by human resources | Three consecutive years of the trade association's turned-away-applications series, with the stated reasons being faculty, clinical sites, classrooms, preceptors, and budget. **But: the publisher is a trade association, and the larger the number turned away the better for its appropriations case; it publishes "applications," not "applicants"; master's and doctoral levels are about a fifth of the total, so using the total to describe the bachelor's-level bottleneck overstates it by about 22%** |
| Same as above | But "the bottleneck is worsening" **does not hold** | The same page states that the 2025 faculty vacancy rate of 7.2% is **below** the ten-year average of 7.64%. **These two numbers must always appear together** |
...
| **The criterion itself** | — | A license that binds after graduation (the bar exam, the pharmacist exam) imposes no constraint on admissions; only a constraint at the entrance restrains overheating. And accrediting bodies loosen procyclically during a boom. **This is a product of this article's reasoning, not any institution's conclusion** |
...
| Field / direction | Position | Basis |
|---|---|---|
| **China: computer science and technology, software engineering** | Penetration already extremely high | 955 and 661 institutions offering them; more than 100,000 graduates a year between them; the most new program instances of any category in 2018–2022. **Administrative records, not subject to sampling problems — the strongest cell in this section** |
| **China: artificial intelligence** | The label is no longer scarce | As of the 2024 approval cycle, about 626 undergraduate institutions had filed this program, roughly half of the national total. **The only robust implication is that the label itself is no longer scarce. It cannot tell you anything about supply and demand for jobs in 2030** |
| **US: computer and information sciences** | Two complete cycles already run; currently in the third at a high level | Forty-year series: two peak-to-trough declines of −42% and −36%; the third ascent is +186% from the trough; the degree pipeline has a four-year delay |
| **US law** | The only case to complete all four steps; currently a divergence of "supply re-expanding + demand softening" | The trough was computed independently by two institutions and two questionnaires; two-thirds of the repair came from the denominator; the fall 2025 first-year class is the largest since 2012 |
| **China: teacher-training fields (the establishment-post channel)** | One channel is tightening, but the magnitude is not credible | Three independent bases point the same way, and a dedicated search found no contrary provincial announcement. **No magnitude is given; one province cannot be extrapolated to the country** |
| **Resource extraction and heavy industry** | A sample of historical reversal | The nine programs given green cards and recommended for expanded admissions by a commercial firm in 2010–2011 are today prime "avoid" territory |
| **Weaknesses of the criterion itself** | — | The shape "overheating → collapse" has historical evidence; its parameters do not: the widely cited "four-year cycle" comes from a two-page appendix containing no statistical estimate of any kind; and students' response to wage signals is weak and slow |
...
## How to check what this article claims
...
- **It counts schools, not people.** The batch documents contain no enrollment plans; whether a program instance admits 30 students a year or 300 is not written in them. A fall in the number of schools and a rise in total enrollment can happen at the same time.
- **Penetration is already near half.** As of the 2024 batch, about 626 undergraduate institutions had filed this program, roughly half the national total. **Less than half the pool remains, so a year-on-year decline in additions is the default result of a diffusion process and has nothing to do with the market.**
- **It is highly sensitive to the policy cycle.** New schools by year: 35 in 2018, about 180 in 2019, 130 in 2020, 95 in 2021, 59 in 2022, 38 in 2023, **rebounding to 91 in 2024.** If you read the decline as "the market cooling," you would have to read 2020 as the turning point for AI employment in China — **and those five years were precisely the years of most explosive growth in AI jobs. This reading has already been wrong once in its own history.** And the 2024 rebound corresponds precisely in time to "AI Plus" entering the Government Work Report — **that is a policy pulse, not a market signal.**
- **It lags by a generation.** A program instance approved this year enrolls in 2026 and graduates in 2030.
...
**One very common misreading**: "the job requires a bachelor's degree" corresponds to about +25% in wages, and "the job matches the field" corresponds to a further +5% **on top of that** — **they are two nested thresholds, not competitors, and the ratio of coefficients is not a ratio of importance.**

---

### A few China-side substitute signals that are closer to you

| Signal | Why it is better | Where to look |
|---|---|---|
| **Enrollment plan headcounts by program** | Corresponds directly to headcount supply, with no "school count ≠ headcount" mismatch | Each province's admissions examination authority's annual admissions plan book, countable school by school |
...
- **Official ten-year occupational projections are credible on the broad direction and not credible on detailed occupational rankings — and choosing a major happens precisely at the detailed level.** So this article gives no ranking, and wherever you are given one, you may ask what makes that source better at a resolution where even the officials do poorly.

- **"Occupation X will have a gap of XX0,000 over ten years" is the most deceptive number in this whole article.** 97% of it comes from people leaving, and 56% of those departures are moves into a different line of work. **A large gap almost never means abundant opportunity; it mainly means people leave fast.**

- **On AI, what is known is that hiring cooled; what is unknown is why, by how much, and for how long.** Over the past three years hiring for some white-collar entry-level jobs in the US really has cooled markedly, and multiple data sources show it; but on those three questions no study can currently give a reliable answer. **Any statement offering a precise percentage exceeds what the evidence can support.** Do not let this section argue you into or out of any particular major.

- **To judge whether a field will overheat, look at whether schools can easily open one more program instance, not at whether the trade requires a license.** A license that binds after graduation (the bar exam, the pharmacist exam) imposes no constraint on admissions — law and pharmacy are both textbook oversupply cases. Only a constraint at the entrance (clinical placement slots, preceptors) genuinely restrains overheating, and even then it can only lengthen the lag and flatten the amplitude; it cannot guarantee overheating does not occur.

- **The income gap within a single field is usually no smaller than the gap between fields.** From the bottom 25% to the top 25%, lifetime cumulative earnings typically double or even triple, and that span is wider for lower-earning fields. So the importance of "picking the right major" is systematically overstated by rankings.

- **"Choose this major and you do this for life" — the data itself does not support that premise.** The share of computer science and engineering majors employed in STEM occupations falls from 59% at age 26 to 41% at 50, almost entirely through movement into non-STEM managerial roles. Return decay is a property of occupations, not of majors.

- **Watch who is talking, and who benefits.** This is not conspiracy thinking; it is basic craft in reading data: MyCOS is a NEEQ-listed commercial company with about 85% of revenue from data monitoring, and its paying customers are the very universities it evaluates; the American Association of Colleges of Nursing is a trade association, and the larger the number of applications turned away the better for its appropriations case — and it publishes "applications," not "applicants"; that nurse turnover report comes from a nurse recruitment company, with a sales phone number printed right in the text; and those "teacher recruitment halved in province X" social media accounts monetize directly off manufacturing anxiety about choosing a major. **But this heuristic is not one-directional** — the most systematic rebuttal arguing that "AI is not killing entry-level jobs" was written by Google's chief economist. **Interests tell you to ask one more question; they do not tell you what the answer is.**

- **The real China–US difference is reproducibility, not the level of the numbers.** China has employment data by field and uses it for administrative action (red and yellow cards, orders to suspend admissions, program abolition), but publishes only rankings and never the values; US by-field data come from a public community survey that anyone can download and recompute. So this article compares only direction and structure, and never puts the two sides' numbers in the same sentence — that is the usage most likely to cause real harm.

- **The most practical point: most of the signals you need, you can look up yourself during application season.** Your province's enrollment plan headcounts by program, the year-by-year change in admission ranks for your target program, your provincial program-warning list, and the within-institution comparison inside your target school's employment quality report. All of these are primary, correspond directly to your decision, and are far closer to the choice you actually have to make than any national ranking.
